...
task :bork do
  puts "DEBUG\n"
  padding =  @variables.keys.inject(0) {|max, current| (current.to_s.size.to_i > max.to_i) ? current.to_s.size.to_i : max.to_i }
  @variables.each_pair do |k,v|
    unless v.is_a?(Proc) || v.is_a?(Logger)
      puts "#{k}:".ljust(padding) + " #{v.inspect}\n"
    end
  end
end
...
namespace :deploy do
  ...
  task :symlink_gmap_api_key, :hosts => "#{domain}" do
    ...
    when 'dev', 'development'
      ...
      run "ln -s #{inat_config_shared_path}/#{rails_env}_gmaps_api_key.yml #{latest_release}/config/gmaps_api_key.yml"
    end
  end
  ...
  task :sphinx_start do
    run "cd #{latest_release} && rake thinking_sphinx:start RAILS_ENV=#{rails_env}"
  end
  
  desc "Executes rake thinking_sphinx:stop, which stops the searchd daemon"
  task :sphinx_stop do
    run "cd #{latest_release} && rake thinking_sphinx:stop RAILS_ENV=#{rails_env}"
  end
  ...
  task :sphinx_index do
    run "cd #{latest_release} && rake thinking_sphinx:index RAILS_ENV=#{rails_env}"
  end
  ...
  task :sphinx_configure do
    run "cd #{latest_release} && rake thinking_sphinx:configure RAILS_ENV=#{rails_env}"
  end
  ...
  task :copy_gmap_api_key, :hosts => "#{domain}" do
    ...
    when 'dev', 'development'
	  ...
	  run "test -e #{inat_config_shared_path}/#{rails_env}_gmaps_api_key.yml || cp #{latest_release}/config/gmaps_api_key.yml.example #{inat_config_shared_path}/#{rails_env}_gmaps_api_key.yml"
    end
  end
